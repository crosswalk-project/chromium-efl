...
class utc_blink_ewk_back_forward_list_previous_item_get : public utc_blink_ewk_base
{
protected:
...
const char* const utc_blink_ewk_back_forward_list_previous_item_get::TEST_URL1 = "ewk_history/page1.html";
const char* const utc_blink_ewk_back_forward_list_previous_item_get::TEST_URL2 = "ewk_history/page2.html";
const char* const utc_blink_ewk_back_forward_list_previous_item_get::TEST_URL3 = "ewk_history/page3.html";

TEST_F(utc_blink_ewk_back_forward_list_previous_item_get, POS_TEST)
{
  ...
  // get prev item and check URL, original URL and title
  Ewk_Back_Forward_List_Item *item = ewk_back_forward_list_previous_item_get(list);
  ...
  // get prev item and it should be null
  ASSERT_EQ(NULL, ewk_back_forward_list_previous_item_get(list)) << "there should not be any previous item, but there is!";
...
TEST_F(utc_blink_ewk_back_forward_list_previous_item_get, EMPTY_TEST)
{
  Ewk_Back_Forward_List *list = ewk_view_back_forward_list_get(GetEwkWebView());
  ASSERT_TRUE(list);
  ASSERT_EQ(NULL, ewk_back_forward_list_previous_item_get(list));
}

TEST_F(utc_blink_ewk_back_forward_list_previous_item_get, NULL_TEST)
{
  ASSERT_EQ(NULL, ewk_back_forward_list_previous_item_get(NULL));
}
